<div class="w-full bg-bgSec sticky top-0 z-30 ">
    
  <!-- categories -->
  @if( !empty($navCategories) )
  <div class="w-full bg-white">
      <div class="mx-auto lg:w-navWidth  min-w-profie overflow-x-scroll lg:overflow-hidden flex">
        <ul class="text-gray-900 capitalize flex justify-between w-full flex-shrink-0">
          @foreach($navCategories as $category)
          <li class="px-3 cat-width hover:bg-bgSec hover:text-gray-50 py-2 rounded font-medium text-center">
              <a href="{!! route('search').'?query='.$category !!}" class="font-bold">
                {!! $category !!}
              </a>
          </li>
          @endforeach
...
<div class="flex items-center lg:order-3 order-2 w-1/2 lg:w-iconW justify-end pb-1">
  ...
  <a href="{!! auth()->user() ? route('user-profile') : route('login') !!}" class="p-1 lg:p-5 lg:inline">
    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 stroke-current" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<!-- slide text -->
<div class="w-full height bg-bgSec mt-2 border-t-2 border-gray-400 height border-b-2 ">
